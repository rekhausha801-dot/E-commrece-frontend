...
const Home = () => {
  ...
  return (
    <div className="home-container">
      ...
      <Cards />
      <TrendyCollection />
      <div className="brands-section" style={{ marginTop: '0', marginBottom: '20px' }}>
        ...
        <div className="products-grid" style={{ padding: '0 5vw', marginTop: '30px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px' }}>
          {[
            ...
            <div key={product.id} className="product-card" style={{ border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', borderRadius: '12px', background: '#fff', overflow: 'hidden' }}>
                <div className="card-image-wrap" style={{ position: 'relative' }}>
                  ...
                  <button className="wishlist-btn" onClick={() => toggleWishlist(product.id)} style={{ top: '10px', right: '10px', background: 'white', border: 'none', borderRadius: '50%', width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 5px rgba(0,0,0,0.1)', cursor: 'pointer', position: 'absolute', zIndex: 2 }}>
                    <Heart 
                      size={15} 
                      fill={wishlist.includes(product.id) ? "#ff4d4f" : "none"} 
                      color={wishlist.includes(product.id) ? "#ff4d4f" : "#666"} 
                ...
                <div className="card-info" style={{ padding: '12px', textAlign: 'left' }}>
                  <h3 className="card-title" style={{ fontSize: '16px', fontWeight: 'bold', color: '#002f5c', marginBottom: '4px' }}>{product.title}</h3>
                  
                  <div className="card-rating" style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '8px' }}>
                  ...
                  <div className="card-price" style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                    <span className="price-new" style={{ fontSize: '20px', fontWeight: 'bold', color: '#111' }}>{product.price}</span>
                    <span className="price-old" style={{ fontSize: '14px', color: '#999', textDecoration: 'line-through' }}>{product.originalPrice}</span>
                    <span className="price-discount" style={{ fontSize: '14px', fontWeight: 'bold', color: '#00a388' }}>{product.discount}</span>
                  </div>
                  
                  <div className="card-actions">
                    <button className="explore-btn" style={{ width: '100%', background: '#d3b585', color: '#3e2a14', border: 'none', borderRadius: '8px', padding: '12px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '14px', transition: 'all 0.3s' }} onMouseOver={(e) => { e.currentTarget.style.background = '#c2a170'; }} onMouseOut={(e) => { e.currentTarget.style.background = '#d3b585'; }}>
                      Explore Collection 
      ...
      <div className="new-arrival-section" style={{ padding: '60px 0 80px', background: '#faf9f6', textAlign: 'center' }}>
        ...
        <div className="products-grid" style={{ padding: '0 5vw', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px', textAlign: 'left' }}>
          {[
            ...
            <div key={product.id} className="product-card" style={{ border: 'none', boxShadow: 'rgba(0, 0, 0, 0.24) 0px 3px 8px', borderRadius: '12px', background: '#fff', overflow: 'hidden' }}>
                <div className="card-image-wrap" style={{ position: 'relative', aspectRatio: '1/1', overflow: 'hidden' }}>
                  {product.badge && (
                  ...
                  <button className="wishlist-btn" onClick={() => toggleWishlist(product.id)} style={{ top: '12px', right: '12px', background: 'white', border: 'none', borderRadius: '50%', width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 5px rgba(0,0,0,0.1)', cursor: 'pointer', position: 'absolute', zIndex: 2 }}>
                    <Heart 
                      size={15} 
                      fill={wishlist.includes(product.id) ? "#ff4d4f" : "none"} 
                      color={wishlist.includes(product.id) ? "#ff4d4f" : "#666"} 
                ...
                <div className="card-info" style={{ padding: '16px', textAlign: 'left' }}>
                  <h3 className="card-title" style={{ fontSize: '15px', fontWeight: 'bold', color: '#222', marginBottom: '6px' }}>{product.title}</h3>
                  
                  <div className="card-rating" style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '15px' }}>
                    <div className="stars" style={{ display: 'flex' }}>
                      {[1, 2, 3, 4, 5].map((_, i) => (
                         <Star key={i} size={12} fill={i < product.rating ? "#d4af37" : "#e0e0e0"} color={i < product.rating ? "#d4af37" : "#e0e0e0"} />
                      ))}
                    </div>
                    <span className="reviews" style={{ fontSize: '12px', color: '#888', marginLeft: '2px' }}>({product.reviews})</span>
                  </div>
                  
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                    <span className="price-new" style={{ fontSize: '18px', fontWeight: 'bold', color: '#111' }}>{product.price}</span>
                    <button style={{ width: '32px', height: '32px', borderRadius: '50%', background: product.color, border: 'none', color: 'white', display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer', boxShadow: '0 2px 5px rgba(0,0,0,0.2)' }}>
                      <ShoppingCart size={14} />
                    </button>
                  </div>
                  
                  <button className="buy-now-full" style={{ width: '100%', background: '#8f7a5b', color: 'white', border: 'none', borderRadius: '6px', padding: '10px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '13px' }}>
                    <ShoppingBag size={14} />
                    BUY NOW
                  </button>
                </div>
